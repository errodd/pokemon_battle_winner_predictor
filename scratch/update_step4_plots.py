...
cells_list = [
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "1. **Repetición Exacta (Fila Duplicada Idéntica):**  \n",
    ...
    "   - **Ejemplo Concreto:**  \n",
    "     - Registro #1: `First = 266 (Larvitar)`, `Second = 298 (Marill)`, `Winner = 298 (Marill)`\n",
    "     - Registro #450: `First = 266 (Larvitar)`, `Second = 298 (Marill)`, `Winner = 298 (Marill)`  \n",
    "     *(Son 2 filas 100% idénticas en el archivo CSV).*  \n",
    "\n",
    "2. **Repetición por Pareja de Contendientes (Reenfrentamiento / Pair Re-match):**  \n",
    "   - **Definición:** Ocurre cuando la misma pareja de Pokémon se enfrenta en múltiples ocasiones en el dataset. Nos permite evaluar la **estocasticidad vs determinismo**: ¿el ganador siempre es el mismo en cada reenfrentamiento, o el resultado cambia?\n",
    "   - **Ejemplo Concreto:**  \n",
    "     - Combate #1: `Larvitar` vs `Marill` $\\rightarrow$ Ganador: `Marill`  \n",
    "     - Combate #1200: `Larvitar` vs `Marill` $\\rightarrow$ ¿Ganador: `Marill` (100% Determinista) o `Larvitar` (Variable/Estocástico)?"
   ]
  },
  {
   "cell_type": "code",
   ...
   "source": [
    ...
    "exact_dup_df['Winner_Is_First'] = (exact_dup_df['Winner'] == exact_dup_df['First_pokemon']).astype(int)\n",
    "\n",
    "# 2. Identificación de reenfrentamientos por pareja (subset=['First_pokemon', 'Second_pokemon'])\n",
    "pair_dup_df = combats_raw[combats_raw.duplicated(subset=['First_pokemon', 'Second_pokemon'], keep=False)].copy()\n",
    "pair_dup_df['Winner_Is_First'] = (pair_dup_df['Winner'] == pair_dup_df['First_pokemon']).astype(int)\n",
    "\n",
    "# 3. Evaluación de determinismo: verificar la cantidad de ganadores distintos por cada pareja\n",
    "pair_groups = pair_dup_df.groupby(['First_pokemon', 'Second_pokemon'])['Winner'].nunique()\n",
    "deterministic_pairs_count = (pair_groups == 1).sum()\n",
    "stochastic_pairs_count = (pair_groups > 1).sum()\n",
    "\n",
    "print(f\"Filas exactamente duplicadas en combats.csv: {exact_dups_count} ({exact_dups_count / len(combats_raw) * 100:.2f}%)\")\n",
    "print(f\"Total de filas en enfrentamientos repetidos por pareja: {len(pair_dup_df)} combates\")\n",
    "print(f\"Parejas únicas con reenfrentamientos: {len(pair_groups)}\")\n",
    "print(f\" - Parejas con resultado 100% DETERMINISTA (siempre gana el mismo): {deterministic_pairs_count} ({deterministic_pairs_count/len(pair_groups)*100:.1f}%)\")\n",
    "print(f\" - Parejas con resultado VARIABLE / ESTOCÁSTICO: {stochastic_pairs_count} ({stochastic_pairs_count/len(pair_groups)*100:.1f}%)\")"
   ]
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "### 6.2.1 Análisis Gráfico de Distribución y Determinismo en Combates Repetidos\n",
    "\n",
    "Generamos una figura comparativa con dos gráficos:\n",
    "1. **Distribución de Ganadores en Combates Duplicados:** Muestra cuántos combates duplicados terminaron con la victoria del `First_Pokemon` (1) frente al `Second_Pokemon` (0).\n",
    "2. **Evaluación de Determinismo por Pareja:** Muestra si el ganador varía o si es 100% constante en cada reenfrentamiento."
   ]
  },
  {
   "cell_type": "code",
   ...
   "outputs": [],
   "source": [
    "# Figura con 2 subgráficos para el análisis de duplicación\n",
    "fig, axes = plt.subplots(1, 2, figsize=(14, 5.5))\n",
    "\n",
    "# Gráfico 1: Distribución del resultado (Winner_Is_First) en filas duplicadas\n",
    "sns.countplot(\n",
    "    data=exact_dup_df, \n",
    "    x='Winner_Is_First', \n",
    "    ax=axes[0], \n",
    ...
    "    legend=False\n",
    ")\n",
    "axes[0].set_title('Distribución de Ganador en Filas Duplicadas', fontsize=13, fontweight='bold')\n",
    "axes[0].set_xticks([0, 1])\n",
    "axes[0].set_xticklabels(['Gana Second Pokemon (0)', 'Gana First Pokemon (1)'])\n",
    "axes[0].set_xlabel('Resultado del Combate')\n",
    "axes[0].set_ylabel('Cantidad de Registros Duplicados')\n",
    "\n",
    "# Anotaciones de porcentaje sobre las barras\n",
    "total_exact = len(exact_dup_df)\n",
    "for p in axes[0].patches:\n",
    "    height = p.get_height()\n",
    "    if height > 0:\n",
    "        axes[0].annotate(\n",
    "            f\"{int(height):,} ({height / total_exact * 100:.1f}%)\",\n",
    "            (p.get_x() + p.get_width() / 2., height / 2),\n",
    "            ha='center', va='center', color='white', fontweight='bold', fontsize=11\n",
    "        )\n",
    "\n",
    "# Gráfico 2: Tasa de Determinismo en Parejas Repetidas\n",
    "det_series = pd.Series({\n",
    "    '100% Determinista\\n(Mismo Ganador)': deterministic_pairs_count,\n",
    "    'Variable / Estocástico\\n(Ganador Cambia)': stochastic_pairs_count\n",
    "})\n",
    "\n",
    "colors = ['#3498DB', '#E74C3C']\n",
    "wedges, texts, autotexts = axes[1].pie(\n",
    "    det_series, \n",
    "    labels=det_series.index, \n",
    "    autopct='%1.1f%%', \n",
    "    colors=colors,\n",
    "    startangle=90, \n",
    "    explode=(0.08, 0),\n",
    ...
    ")\n",
    "axes[1].set_title('Determinismo en Reenfrentamientos por Pareja', fontsize=13, fontweight='bold')\n",
    "\n",
    "plt.tight_layout()\n",
    "plt.show()"
   ]
  ...
  },
  {
   ...
   "source": [
    ...
    ">\n",
    "> 2. **Determinismo Absoluto en Reenfrentamientos por Pareja (100% Determinista):**  \n",
    ">    - El análisis gráfico demuestra de forma contundente que el **100% de las 1,906 parejas** de Pokémon que se enfrentaron más de una vez registraron **EXACTAMENTE EL MISMO GANADOR** en todas sus batallas.  \n",
    ">    - *Conclusión del Dominio:* Esto demuestra empíricamente que el simulador de combates que generó los datos es **completamente determinista** (basado en una regla matemática fija de atributos y ventajas de tipo, sin componente de aleatoriedad o suerte). Por ende, los 1,952 combates duplicados representan simulaciones legítimas de frecuencia y **no deben ser eliminados**.\n",
    ">\n",
...
print('Updated Section 6.2 with detailed examples and plots successfully!')
