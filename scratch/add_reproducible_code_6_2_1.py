...
cells_list = [
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "Una vez identificados y graficados los dos grupos en el código anterior, ejecutamos una celda de código reproducible para cuantificar empíricamente la superposición de Pokémon y los factores determinantes de velocidad y nivel de poder entre ambos grupos."
   ]
  },
  {
   "cell_type": "code",
   ...
   "outputs": [],
   "source": [
    "# === CÓDIGO REPRODUCIBLE DE AUDITORÍA COMPARATIVA SECCIÓN 6.2.1 ===\n",
    ...
    "\n",
    "def compute_group_metrics(pairs_df, pkmn_data):\n",
    ...
    "    return m\n",
    "\n",
    "det_metrics = compute_group_metrics(det_pairs_df, pokemon_stats_df)\n",
    "var_metrics = compute_group_metrics(var_pairs_df, pokemon_stats_df)\n",
    "\n",
    "# 3. Construcción e impresión de la tabla resumen comparativa\n",
    "comp_summary_df = pd.DataFrame({\n",
    "    'Métrica de Dominio': [\n",
    "        'Parejas Únicas Reenfrentadas',\n",
    "        'Pokémon Únicos Involucrados',\n",
    "        'Porcentaje de Empates en Velocidad (Speed_1 == Speed_2)',\n",
    "        'Diferencia Mediana de Velocidad (Speed)',\n",
    "        'Diferencia Mediana de Stats Totales (Total_Stats)'\n",
    "    ],\n",
    ...
    "        f\"{len(var_pairs_df):,}\",\n",
    "        f\"{len(pkmn_var):,} (Superposición: {len(overlap_pkmn)/len(pkmn_var)*100:.1f}%)\",\n",
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "> **Conclusiones del Análisis Comparativo de Subgrupos:**\n",
    ">\n",
...
print('Added reproducible code cell to Section 6.2.1 successfully!')
