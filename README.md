...
│   └── pokemon_combats_unified.csv       # Unified analytical table: 50,000 battles × 29 cols (derived, tracked)
│
├── scripts/                              # Data / feature-engineering pipeline
│   └── merge.py                          # Builds the unified table from the raw CSVs (see §3.3)
│
...
The EDA reads from a **single consolidated table**, `data/pokemon_combats_unified.csv`, generated by `scripts/merge.py` from the raw CSV files:

- Each row is one historical battle, with the stats of both fighters merged twice using `_first` / `_second` suffixes.
- The binary target `Target_First_Wins` is derived (`1` = `First_pokemon` wins).
- Engineered differential features are added: `Speed_diff`, `Attack_diff`, and `Defense_diff`.

```powershell
# The script resolves its CSV paths from the current working directory,
# so run it from the data/ folder
python ../scripts/merge.py
```

> [!NOTE]
> The unified CSV is **tracked** in the repository, so it does not need to be regenerated to run the notebook. It should be re-generated and committed whenever the feature definitions in `merge.py` change.
